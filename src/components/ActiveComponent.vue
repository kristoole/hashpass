<template>
  <div class="dflex flex-col" style="padding: 2rem">
    <transition name='fade'>
      <div v-show="running" class='skull-right'></div>
    </transition>
    <transition name='fade'>
      <div v-show="running" class='skull-left'></div>
    </transition>
    ...
    <div class="chart">
      <div class="chart__left text-right">
        ...
        <p class="flex1"><span>Time Started:</span>
          <vue-typer v-if="getRunning[0]"
            :text="progress.time_started.toString()"
            :pre-erase-delay='3000'
            :erase-delay='50'>
          </vue-typer>
        </p>
        <p class="flex1"><span>Time Estimeated:</span>
          <vue-typer v-if="getRunning[0]"
            :text="progress.time_estimated.toString()"
            :pre-erase-delay='3000'
            :erase-delay='50'>
          </vue-typer>
        </p>
      ...
      <div class="chart__right text-left">
        ...
        <p class="flex1 dnone"><span>Speed Dev #2:</span>
          <vue-typer v-if="getRunning[0]"
            :text="progress.speed_dev_2.toString()"
            :pre-erase-delay='3000'
            :erase-delay='50'>
          </vue-typer>
       </p>
        <p class="flex1"><span>Rejected:</span>
          <vue-typer v-if="getRunning[0]"
            :text="progress.rejected.toString()"
            :pre-erase-delay='3000'
            :erase-delay='50'>
          </vue-typer>
        </p>
        <p class="flex1"><span>Restore Point:</span>
          <vue-typer v-if="getRunning[0]"
            :text="progress.restore_point.toString()"
            :pre-erase-delay='3000'
            :erase-delay='50'>
          </vue-typer>
        </p>
    ...
    <h2 class='text-center'>STDOUT</h2>
  ...
  export default {
    ...
    mounted() {
      ...
      setInterval(function(){
        ...
        chart.setTitle(null,{text: vm.status});
      }, 5000);
    },
    computed: {
      ...
      hash: function() {
        ...
        if (running) {
          console.log('running hash', running.hash)
          return running.hash
        }
        else {
      ...
      running: function() {
        console.log('running', store.getters.getIsRunning);
